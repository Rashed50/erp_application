...
class SalesPaymentReceived extends Controller
{
    ...
    public function create()
    {
        ...
        $data['unpaid_sales'] = ChartofaccSalesRecord::select('sr_auto_id','sr_invoice_no','sr_grand_total_amount')->whereIn('paid_type',[0,5])
                                ...
                                ->get();
        return view('account::pages.sales.sales_payment.create', [
            'data' => $data
        ]);
    }
    ...
    public function search(Request $request){
        ...
        try{
            ...
            $salesService = new SalesService();
            $data = $salesService->searchSalesPaymentReceivedRecords($request->customer_id, $request->from_date, $request->to_date);
            return response()->json(['success'=>true,'data'=>$data,'form'=>$request->all()],200);

        }catch(\Exception $e){
            Log::error("Error searching sales: " . $e->getMessage());
            return response()->json(['success'=>false,'message'=>'Error searching sales'],500);
        }
    }
