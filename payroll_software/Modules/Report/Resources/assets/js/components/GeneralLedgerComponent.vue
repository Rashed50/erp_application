<template>
    <div class="card" style="padding:0px; margin: 0px;">
        <!-- <div class="card-header"> -->
            <h4 class="text-center fw-bold" style="color: blue">Ledger Report</h4>
        <!-- </div> -->
        <div class="card-body">
            <form>
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group mb-3">
                            <label for="account_id">Accountt</label>
                            <Multiselect
                    ...
                    <div class="col-md-3">
                        <div class="form-group mb-3">
                            <label for="account_id">Project</label>
                            <Multiselect
                            ...
                            >
                                ...
                                <template #nooptions>
                                    <span class="text-muted">No Project available</span>
                                </template>
                            </Multiselect>
                        </div>
                    </div>
    ...
    computed: {
        ...
        accountOptions() {
            ...
            return  this.accounts
                .filter(account => account.acct_type_id === 5) // Step 1: Filter only expense accounts
                .map(account => ({                             // Step 2: Format
                    label: account.chart_of_acct_name,
                    value: account.chart_of_acct_id
                }));
        },
        projectOptions() {
            if (!this.data_for_form.projects || !Array.isArray(this.data_for_form.projects)) return [];

            return  this.data_for_form.projects
               // .filter(aproject => account.acct_type_id === 5) // Step 1: Filter
                .map(aproject => ({                             // Step 2: Format
                    label: aproject.proj_name,
                    value: aproject.proj_id
                }));
        },
        selectedAccountName() {
    ...
    methods: {
        ...
        downloadReport() {
            ...
            if (this.form.report_type === 'PDF') {
                const url = `/admin/report/sales-purchase-summary?` + new URLSearchParams(this.form).toString();
        ...
        async generatePdfInNewTab() {
            try {
                const response = await axios.post("/admin/report/sales-purchase-summary", this.form);
                if (response.data.success) {
