...
<body>
    ...
    <main>
        <div style="text-align:left; font-weight:bold; font-size:15px; margin-bottom:8px;">
            Supplier Report from {{ \Carbon\Carbon::parse($start_date)->format('d M Y') }} to {{ \Carbon\Carbon::parse($end_date)->format('d M Y') }}
        </div>

        @if(isset($previous_balance) && $previous_balance != 0)
            <div style="text-align:right; font-weight:bold; font-size:14px; margin-bottom:10px;">
                Previous Balance till {{ \Carbon\Carbon::parse($start_date)->subDay()->format('d M Y') }}: {{ number_format($previous_balance, 2) }}
            </div>
        @endif
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-right">
                        {{ (!empty($item['paid']) && $item['paid'] != 0) ? number_format($item['paid'], 2) : '' }}
                    </td>
                    <td class="text-right">
                        {{ (!empty($item['amount']) && $item['amount'] != 0) ? number_format($item['amount'], 2) : '' }}
                    </td>
                    <td class="text-right">
                        {{ (!empty($item['due']) && $item['due'] != 0) ? number_format($item['due'], 2) : '' }}
                    </td>
                </tr>
                @endforeach
            </tbody>
            <tfoot>
                <tr>
                    ...
                    <th class="text-right">{{ number_format($totals['total_paid'], 2) }}</th>
                    <th class="text-right">{{ number_format($totals['total_amount'], 2) }}</th>
                    <th class="text-right">{{ number_format($totals['total_due'], 2) }}</th>
                </tr>
            </tfoot>
        </table>
    </main>
