<template>
  <div>
    <!-- Search Component -->
    <EmployeeSearchComponent @searching_result="handleSearchingResult"></EmployeeSearchComponent>

    <!-- Update Employee Reference Information Card -->
    <div v-if="searched_employee" class="card shadow-sm border rounded-3 mt-4">

      <!-- Card Header -->
      <div class="card-header bg-light text-center py-3" style="border-bottom: 1px solid #ededed;">
        <h5 class="mb-0">
          UPDATE EMPLOYEE AGENCY INFORMATION
        </h5>
      </div>

      <!-- Card Body & Form -->
      <div class="card-body px-5 py-4">
        <form @submit.prevent="submitForm">
          <input type="hidden" v-model="form.emp_auto_id" />

          <!-- 1. Agency Name Dropdown Field -->
          <div class="row mb-3 align-items-center">
            <label class="col-sm-4 text-end font-weight-bold text-dark pe-3">
              Agency Name: <span class="text-danger">*</span>
            </label>
            <div class="col-sm-6">
              <select class="form-select" v-model="form.emplyoeeAgency" required>
                <option value="">Select Agency Name</option>
                <option v-for="agency in agencies" :key="agency.agc_info_auto_id" :value="agency.agc_info_auto_id">
                  {{ agency.agc_title }}
                </option>
              </select>
            </div>
          </div>

          <!-- 2. Reference Person Name Field -->
          <div class="row mb-3 align-items-center">
            <label class="col-sm-4 text-end font-weight-bold text-dark pe-3">
              Reference Person Name:
            </label>
            <div class="col-sm-6">
              <input type="text" placeholder="Input Reference Name or Employee ID" class="form-control"
                v-model="form.ref_employee_id">
            </div>
          </div>

          <!-- 3. Reference Contact Field -->
          <div class="row mb-3 align-items-center">
            <label class="col-sm-4 text-end font-weight-bold text-dark pe-3">
              Ref. Contact:
            </label>
            <div class="col-sm-6">
              <!-- input type changed to "tel" for uniform telephone interactions -->
              <input type="tel" placeholder="Input Reference Contact Number Here" class="form-control"
                v-model="form.ref_contact_no">
            </div>
          </div>

          <!-- 4. Remarks Field -->
          <div class="row mb-4 align-items-center">
            <label class="col-sm-4 text-end font-weight-bold text-dark pe-3">
              Remarks:
            </label>
            <div class="col-sm-6">
              <input type="text" placeholder="Input Remarks Here" class="form-control" v-model="form.remarks">
            </div>
          </div>

          <hr class="text-muted my-4">

          <!-- Action Button Footer -->
          <div class="d-flex justify-content-end me-sm-5 pe-md-4">
            <button type="submit" :disabled="is_saving" class="btn text-white px-4 py-2"
              style="background-color: #2c3e50; border-radius: 4px; font-weight: 500;">
              <i v-if="is_saving" class="fas fa-spinner fa-spin me-2"></i>
              {{ is_saving ? 'Updating...' : 'Update' }}
            </button>
          </div>
        </form>
      </div>

    </div>
  </div>
</template>
<script>
import axios from 'axios';
import { toast } from 'vue3-toastify';
import EmployeeSearchComponent from './SearchComponent.vue';
import { ReferenceUpdateAPI } from '../../routes.js';

export default {
  components: { EmployeeSearchComponent },
  props: {
    agencies: {
      type: Object,
      required: true,
    }
  },

  data() {
    return {
      searched_employee: null,
      is_saving: false,

      form: {
        emp_auto_id: '',
        ref_employee_id: '',
        ref_contact_no: '',
        remarks: '',
        emplyoeeAgency: '',

      }
    }
  },

  methods: {


    handleSearchingResult(employee) {
      if (employee == null) return;



      this.searched_employee = employee;
      this.form.emp_auto_id = employee.emp_auto_id;
      this.form.ref_contact_no = employee.contact_no;
      this.form.ref_employee_id = employee.ref_employee_id;
      this.form.emplyoeeAgency = employee.agc_info_auto_id;
      this.form.remarks = employee.remarks;
     


      console.log('Data Received Successfully', employee);
    },

    async submitForm() {
      try {
        if (!this.form.emp_auto_id) {
          toast.error("Please Search an Employee then Try to update ");
          return;
        }
        this.is_saving = true;
        let formData = new FormData();

        formData.append('emp_auto_id', this.form.emp_auto_id);
        formData.append('ref_contact_no', this.form.ref_contact_no);
        formData.append('ref_employee_id', this.form.ref_employee_id);
        formData.append('emplyoeeAgency', this.form.emplyoeeAgency);
        formData.append('remarks', this.form.remarks);


        const response = await axios.post(ReferenceUpdateAPI, formData, {
          // headers: { 'content-type': 'multipart/form-data' }

        });

        debugger;
        if (response.data.status === 200) {
          toast.success(response.data.message || 'Updated successfully!');
        } else {
          toast.error('Update failed: ' + response.data.message);
        }
      } catch (error) {
        console.error(error);
        toast.error('Error occurred , Please Try Again: ' + error.message);
      } finally {
        this.is_saving = false;
      }
    },

  }
}
</script>