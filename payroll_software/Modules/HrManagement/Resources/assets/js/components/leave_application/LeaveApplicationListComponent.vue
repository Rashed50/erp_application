        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-md-6">
                            <h4 class="card-title">Employee Leave Approval</h4>
                        </div>
                            ...
                            <tbody>
                                ...
                                <tr v-else v-for="(record, index) in records" :key="record.leav_auto_id">
                                    ...
                                    <td>
                                        ...
                                        <a href="#" @click.prevent="rejectLeave(record.leav_auto_id)"
                                           v-if="canReject" title="Delete">
                                            <i class="fa fa-trash fa-lg delete_icon"></i>
                                        </a>
                                        <span v-if="canReject">|</span>
                                        <a :href="getAttachmentUrl(record.leave_paper)" target="_blank" title="View">
                                            <i class="fa fa-eye"></i>
                                        </a>
                                    </td>
                                </tr>
                                    ...
                                    <div class="input-group">
                                        <span class="input-group-btn">
                                            <span class="btn btn-default btn-file btnu_browse">
                                                Browse… <input type="file" @change="handleFileUpload" ref="exitPaperInput">
                                            </span>
                                        </span>
                                        <input type="text" class="form-control" :value="editForm.exit_paper_name" readonly>
                                    </div>
                                </div>
                                <div class="col-sm-4">
                                    <img v-if="editForm.exit_paper_preview" :src="editForm.exit_paper_preview"
                                         class="upload_image" style="max-height: 100px;" />
                                </div>
                            </div>
...
import {LeaveApplicationPendingList,LeaveApplicationUpdateAPI,LeaveApplicationRejectAPI} from '../../routes.js'
...
export default {
    ...
    methods: {
        ...
        async rejectLeave(leav_auto_id) {
            if (!confirm('Are you sure? Once deleted, you will not be able to recover this record!')) {
                return;
            }

            try {
                const response = await axios.get(`${LeaveApplicationRejectAPI}/${leav_auto_id}`);
                if (response.data.status === 200) {
        ...
        handleFileUpload(event) {
            ...
            if (file) {
                this.editForm.exit_paper = file;
                this.editForm.exit_paper_name = file.name;

                const reader = new FileReader();
                reader.onload = (e) => {
                    this.editForm.exit_paper_preview = e.target.result;
                };
