...
<head>
    ...
    <title>{{$employee == null ? "": $employee->employee_id }}-Fiscal-Salary</title>
    <style>
        .loader {
        ...
        * {
        ...
        th,
        td {
            padding: 5px 2px; /* Top,Right,Bottom,left */
        }
...
<body>
    ...
    <div class="main__wrap">
                ...
                <tbody>
                    ...
                    @endforeach
                     ...
                     <tr class="t__total_row">
                        ...
                        <td class="td__gross_salary">S.T+Adv+C.D<br><br> {{round($totalSaudiTax)}}+ {{  round($totalOtherAdvance)}} + {{  round($totalFoodDeduction)}} </td>
        ...
        <section>
            <table id="salary_table">
                <thead>
                    <tr>
                        <td colspan="13"
                            style="text-align: center;background:rgb(138, 219, 118, 0.46);padding-top:5px;padding-bottom:5px;font-size: 13px;font-weight: bold;">
        ...
        <section>
            <br>
            <table style="width: 70%; margin-left: auto; margin-right: auto; font-size:13px;">
                <tbody>

                    @php


                         $iqama_balance = $closed_fiscal_record->balance_amount + $iqamaRenewalTotalExpence  - (  $toal_iqama_expense_deduction_from_salary +  $cashReceiveTotalPaidAmount  );
                        $advance_balance =   $otherAdvanceTotalAmount  - $total_other_advace_deduction_from_salary;
                        $final_balance = round( $iqama_balance  + $advance_balance) ;
                        $total_unpaid_salary_amount = round($totalUnPaidSalaryAmount);
                        if ($final_balance <= 0)
                          {
                            $is_payable = true;
                            $final_balance = ($final_balance *(-1) )+ $total_unpaid_salary_amount;
                          }
                        else {
                             if($final_balance - $total_unpaid_salary_amount < 0 ){
                                $is_payable = true;
                                $final_balance = ($final_balance - $total_unpaid_salary_amount)*-1 ;
                             }
                             else{
                                $is_payable = false;
                                $final_balance = $final_balance - $total_unpaid_salary_amount;
                            }
                        }
                    ...
                    <tr style=" background-color: #f2f2f2;">
                        <td class="sm_td_header" >
                            <!--{{  $closed_fiscal_record->end_year != null ? $closed_fiscal_record->end_year : 'PREVIOUS' }} YEAR-->
                             Closing Balance (B/F)</td>
                        <td class="td__balance_table_gross_salary">
                        {{number_format(  $closed_fiscal_record->balance_amount ,2) }}
                        </td>
                    ...
                    <tr>
                        ...
                        <td class="td__balance_table_gross_salary">{{ number_format(round($iqamaRenewalTotalExpence + $closed_fiscal_record->balance_amount),2) }} </td>
                    ...
                    <tr  style=" background-color: #f2f2f2;">
                        ...
                        <td class="td__balance_table_gross_salary"> {{ number_format(round(  ($iqamaRenewalTotalExpence+$closed_fiscal_record->balance_amount) -   ($toal_iqama_expense_deduction_from_salary + $cashReceiveTotalPaidAmount) ),2) }} </td>
                    </tr>
                   <tr  style=" background-color:rgba(197, 171, 87, 0.932); color:navy; font-weight:bold;">
                        ...
                        <td class="td__balance_table_gross_salary">{{ number_format(round(  $final_balance),2) }} </td>
                    </tr>
                      <tr>
