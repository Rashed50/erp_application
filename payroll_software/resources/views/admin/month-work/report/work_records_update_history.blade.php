...
<head>
    <meta charset="utf-8">
    <title>Timesheet Update History</title>
    <style>
...
<body>
    ...
    <div class="page-container">
        ...
        <main>
            <h4 style="text-align:center; font-weight: bold;">Working Record Update History Month of {{ strtoupper(date("F", mktime(0, 0, 0, $month, 10))) }} {{ $year }}</h4>
            ...
            <tbody>
                    ...
                    @php
                      ...
                      $total_hours = $ar->total_hours + $ar->overtime;
                      $updated_records = $ar->updated_records;
                      $old_records = $ar->old_records;
                      $old_total_hours = 0;

                    @endphp
                    <tr>
                        <td class="td__sn"> {{$loop->iteration}}</td>              
                        <td class="td__sn">{{ $ar->employee_id }}</td>
                        <td class="td__project">{{ $ar->employee_name }}</td>
                        <td class="td__project">{{ $ar->akama_no }}</td>
                        <td class="td__project">{{ $ar->catg_name }}</td>                                               
                        <td class="td__amount" > {{ $ar->total_work_day }}  </td>
                        <td class="td__amount" > {{ round( $ar->total_hours + $ar->overtime )}} ({{ round( $ar->total_hours ) }}+{{ round($ar->overtime) }}) </td>
                        <td>                             
                            @if(count($old_records))                                
                                @foreach($old_records as $or)
                                @php
                                     $old_total_hours =  $or->total_hour + $or->total_overtime;
                                @endphp
                                    {{ $or->total_hour + $or->total_overtime }} ({{ $or->total_hour }} + {{ $or->total_overtime }}) <br>
                                @endforeach                            
                            @endif
                        </td>
                        <td>{{ round(( $ar->total_hours + $ar->overtime) ) - $old_total_hours }}</td> 
                    </tr>
                    @endforeach
