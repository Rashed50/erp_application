...
use App\Http\Controllers\Admin\Helper\{HelperController,UploadDownloadController};
...
class FiscalYearController extends Controller
{
    ...
    function __construct()
    {
         ...
         $this->middleware('permission:employee_fiscal_year_search', ['only' => ['index','searchAnEmployeeOpenClosingAllRecords']]);
         $this->middleware('permission:employee_fiscal_year_edit', ['only' => ['updateAnEmployeeSalaryFiscalYear']]);
         $this->middleware('permission:employee_fiscal_year_file_upload', ['only' => ['updateAnEmployeeSalaryFiscalYear']]);
         $this->middleware('permission:employee_fiscal_year_delete', ['only' => ['deleteAnEmployeeFiscalYear']]);
    }
    ...
    public function searchAnEmployeeOpenClosingAllRecords(Request $request){
        ...
        try{
            ...
            return response()->json(['status'=>200,'success'=>false, 'data' => $records]);
    ...
    public function searchAnEmployeeLastClosingFiscalRecord(Request $request){
          ...
          try{
              ...
              if($isExistRunningYear){
                return response()->json(['status'=>603,'success'=>false, 'message'=>'The Employee Fiscal Year has Already Opened','error' => 'error']);
              }
    ...
    public function updateAnEmployeeSalaryFiscalYear(Request $request){
      try{
                ...
                // 25.4.2026 have to work to protect overlaping of fiscal year closing and starting date with other fiscal year
                //  $isExist = (new FiscalYearDataService())->checkAnEmployeeFiscalYearRecordExistAfterThisDate($request->emp_auto_id, $request->start_date,$request->closing_date);
                // if($isExist){
                //     return response()->json(['success'=>true,'status'=>303,'message'=>'Fiscal Year Exist within This Starting and End Date']);
                // }
        ...
        if($request->operation_type == 1 || $request->operation_type == 0 ){
              ...
              if($request->operation_type == 0 && !(new FiscalYearDataService())->checkAnEmployeeRunningFiscalYearIsAlreadyExist($fiscal_record->emp_auto_id)){

                    return response()->json(['success'=>true,'status'=>303,'message'=>'The Employee has already opened the Fiscal Year']);
              }



              $fiscal_record->end_date = $request->closing_date;
            ...
            if ($request->hasFile('fiscal_closing_file')) {
                $file = $request->file('fiscal_closing_file');
                $file_path = (new UploadDownloadController())->uploadEmployeefiscalClosingSheet($file,$fiscal_record->closing_file);
                (new FiscalYearDataService())->updateAnEmployeeFiscalYearClosingFilePath($request->fiscal_year_auto_id,$file_path);
            }
            ...
            if ($request->hasFile('fiscal_closing_file')) {
                $file = $request->file('fiscal_closing_file');
                $file_path = (new UploadDownloadController())->uploadEmployeefiscalClosingSheet($file,null);
                (new FiscalYearDataService())->updateAnEmployeeFiscalYearClosingFilePath($request->fiscal_year_auto_id,$file_path);
                return response()->json(['success'=>true,'status'=>200,'message'=>'File Uploaded Successfully ','dd'=>$request->all()]);

            }

        } else if($request->operation_type == 4){
            // Reopen Closed Fical year
              $fis_record = (new FiscalYearDataService())->getAnEmployeeFiscalYearRecordByFiscalYearAutoId($request->fiscal_year_auto_id);

              if($fis_record && $fis_record->closing_status == true ){
                    (new FiscalYearDataService())->deleteExistingOpenedFiscalRecordAndReopenAlreadyClosedFiscalYear($fis_record->efcr_auto_id, $fis_record->emp_auto_id,Auth::user()->id);
                    return response()->json(['success'=>true,'status'=>200,'message'=>'The Fiscal Year Reopend Successfully']);
              }
              return response()->json(['success'=>true,'status'=>403,'message'=>'Already Opened']);
