<template lang="html">
    <Breadcrumb title="Supplier Ledger" buttonText="Back Suppliers" :buttonLink="{ name: 'admin_suppliers_list' }"
        buttonIcon="list" />
    ...
    <div class="main-content-wrapper mt-4">
        <div class="container-fluid">
            ...
            <v-card style="padding: 15px; margin-top: 15px;">
                <div class="row">
                    <div class="col-md-3"><strong>Name:</strong> {{ supplier.name }}</div>
                    <div class="col-md-3"><strong>Email:</strong> {{ supplier.email }}</div>
                    <div class="col-md-3"><strong>Phone:</strong> {{ supplier.phone }}</div>
                    <div class="col-md-3">
                        <strong>Status:</strong>
                        <span :class="supplier.active_status ? 'badge bg-success' : 'badge bg-secondary'">
                            {{ supplier.active_status ? 'Active' : 'Inactive' }}
                        </span>
                    </div>
                </div>
                <div class="row mt-2">
                    <div class="col-md-3"><strong>Opening Balance:</strong> {{ Number(supplier.opening_balance ?? 0).toFixed(2) }}</div>
                    <div class="col-md-3"><strong>Current Balance:</strong> {{ Number(supplier.current_balance ?? 0).toFixed(2) }}</div>
                </div>
            </v-card>

            <!-- Add Transaction -->
            <v-card style="padding: 15px; margin-top: 15px;" v-if="can(['supplier-transactions.create'])">
                        ...
                        <div class="col-md-3">
                            <div class="form-group mb-3">
                                ...
                                <select class="form-control" v-model="form.direction" required>
                                    <option value="credit">Credit (increases what's owed)</option>
                                    <option value="debit">Debit (decreases balance, e.g. payment)</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-3">
                ...
                <v-table class="custom-bordered">
                    ...
                    <tbody>
                        ...
                        <tr v-else v-for="item in items" :key="item.id">
                            ...
                            <td class="text-center">
                                <v-btn v-if="item.status && can(['supplier-transactions.delete'])" size="small"
...
const {
    ...
    changePage,
    changePerPage,
} = usePaginatedFetch(`/api/suppliers/${route.params.id}/transactions`)
...
const loadSupplier = async () => {
    try {
        const { data } = await axios.get(`/api/suppliers/${route.params.id}`)
        if (data.success) supplier.value = data.data
    } catch (e) {
        console.error(e)
    }
}
...
const handleAddTransaction = async () => {
    ...
    try {
        const { data } = await axios.post(`/api/suppliers/${route.params.id}/transactions`, payload)
        if (data.success) {
...
const handleReverse = async (item) => {
    const result = await Swal.fire({
        title: 'Are you sure?',
        text: 'This reverses the transaction and restores the supplier balance.',
        icon: 'warning',
    ...
    try {
        const { data } = await axios.delete(`/api/suppliers/${route.params.id}/transactions/${item.id}`)
        if (data.success) {
            toast.success(data.message)
            fetchData()
            loadSupplier()
