<template lang="html">
    <Breadcrumb title="Edit Supplier" buttonText="Back Suppliers" :buttonLink="{ name: 'admin_suppliers_list' }"
        buttonIcon="list" />
                        ...
                        <form @submit.prevent="handleSubmit">
                            <div class="row">
                                ...
                                <!-- Opening balance is fixed at creation time and only moves afterwards
                                     through ledger transactions and purchases, so it is shown for reference only. -->
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
...
const handleSubmit = async () => {
    const resp = await submit(`/api/suppliers/${route.params.id}`, 'put')
...
const loadSupplier = async () => {
    try {
        ...
        if (data.success) {
            ...
            form.vat_no = supplier.vat_no
            form.payment_term = supplier.payment_term
            form.contact_person = supplier.contact_person
            form.contact_person_phone = supplier.contact_person_phone
            form.contact_person_email = supplier.contact_person_email
            form.active_status = supplier.active_status
            openingBalance.value = Number(supplier.opening_balance).toFixed(2)
            currentBalance.value = Number(supplier.current_balance).toFixed(2)
        }
    } catch (e) {
        console.error(e)
    }
