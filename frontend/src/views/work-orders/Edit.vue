<template lang="html">
    <Breadcrumb title="Edit Work Order" buttonText="Back Work Orders" :buttonLink="{ name: 'admin_work_orders_list' }"
        buttonIcon="list" />
...
const handleSubmit = async () => {
    const resp = await submit(`/api/work-orders/${route.params.id}`, 'put')
...
const loadWorkOrder = async () => {
    try {
        const { data } = await axios.get(`/api/work-orders/${route.params.id}`)
        if (data.success) {
            const workOrder = data.data
            form.customer_id = workOrder.customer_id
            form.work_title = workOrder.work_title
            form.work_order_no = workOrder.work_order_no
            form.issue_date = workOrder.issue_date
            form.total_amount = workOrder.total_amount
            form.retention_percent = workOrder.retention_percent
            form.deliver_date = workOrder.deliver_date || ''
